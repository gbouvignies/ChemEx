...
ChemEx is implemented in pure Python, leveraging established open-source libraries such as NumPy, SciPy, Matplotlib, Rich, and pydantic. Deterministic fitting uses bounded SciPy trust-region reflective least squares through ChemEx's native parameter and evaluation stack. Statistics, grid searches, resampling, and MCMC are likewise implemented through ChemEx-owned native paths.
