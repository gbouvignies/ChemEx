...
When a method requests `GRID`, `grid.out` records the aggregate chi-square for
each grid seed. One- and two-dimensional aggregate views are written as
`grid_1d.pdf` and `grid_2d.pdf` when applicable. Fit-component-local GRID files
are execution details and are not published as separate results.
