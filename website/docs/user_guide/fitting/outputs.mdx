...
For `.tc` models, output and restart files use only the canonical `TREF`,
`CS0`/`CS1`, and `DW0`/`DW1` coordinates. See
[Temperature-dependent chemical shifts](temperature_dependent_shifts.md#output-provenance-and-restart).
...
are not cleared.
All Method Steps are validated against the loaded profiles before this clearing
or `run_info/` publication. A rejected plan, including one with an invalid later
step, leaves prior results and provenance untouched.
...
import CestProfile from "@site/static/img/cest_26hz_fit.png";
import CpmgProfile from "@site/static/img/cpmg_800mhz_fit.png";

<figure>
    <img src={CestProfile} alt="CEST profile" width="50%" />
    <img src={CpmgProfile} alt="CPMG profile" width="50%" />
    <figcaption align="center">
        <b>Examples of CEST and CPMG fitting results</b>
    </figcaption>
</figure>
...
Analytically fitted profile normalizations are not included in that field, but
