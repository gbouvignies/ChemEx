...
For any given set of parameters, ChemEx performs a numerical simulation and
calculates the corresponding *χ*\ :sup:`2`. The aim of the fitting process
is to find the parameters that result in a *χ*\ :sup:`2` minimum. This
minimization procedure is carried out using the
`lmfit <https://lmfit.github.io/lmfit-py/>`_ module, which supports all of
the algorithms available in `SciPy <https://scipy.org/>`_.
Unlike most other programs for chemical exchange data analysis, ChemEx does
not fit datasets with analytical equations and therefore most experimental
details (e.g., finite pulse width, off-resonance effects, etc.) can be taken
into account. Besides, for certain types of experiments (e.g., D-CEST/COS-CEST)
or kinetic models (e.g., 3-state exchange model) it is difficult to fit using
analytical equations and, therefore, numerical fitting is the only way to
obtain exchange parameters in such cases.


Running ChemEx
--------------

ChemEx is designed for use on the command line (indicated with ``$``); a
typical command for running ChemEx for data fitting purpose is:
...
Additional arguments can be one of the following:
...
.. tip::
   ...
   possible to provide multiple files using the wildcard character
   (i.e., ``*``), instead of specifying the name of each file
   individually.
...
consists of :confval:`key = value` pairs, :confval:`[section names]`
and :confval:`# comments`. A detailed description of this file format
can be found on the `TOML website <https://toml.io/>`_.
...
Each experiment may also contain certain specific keys, a complete list
...
.. important::
   ...
   properly chosen according to the spin system for each experiment. For
   example, :confval:`G2N` is suitable for experiments based on single-spin
   system, whereas :confval:`G2N-HN` is suitable for experiments based
...
whereas residue-specific parameters can be specified under other sections with
...
.. tip::
   ...
   specified, but also upper and lower bounds during the fitting process.
   Such boundaries can help to prevent parameters wandering off to unrealistic
   values in order to minimize *χ*\ :sup:`2`. However, one should be careful
   not to set too stringent boundaries either as this can result in convergence
   problems. Certain minimization algorithms (i.e., ``brute`` and
   ``diferential_evolution``) require finite bounds on all fitted parameters.

   With the use of boundaries, each parameter key should look like
...
.. note::
   ...
   should match those in experiment file(s); ``"ALL"`` is the default value,
   which indicates that all residues are to be included in the current
   fitting step. Similarly, the :confval:`EXCLUDE` key can be used to
   exclude residues from the analysis.
   .
...
.. tip::
   ...
   quality CEST minor dips to first obtain global parameters
   (p\ :sub:`b`, k\ :sub:`ex`), and then carry out single-residue
...
Multiple states involved in an exchange process are distinguished with
...
* ``statistics.toml``: Contains all statistical information about the
  fitting results, such as *χ*\ :sup:`2`.

* ``Data/``: Contains all data files used for the fitting, which are
  identical to the input data files.

* ``Parameters/``: Contains fitting results as three separate files
...
* ``Plot/``: Contains fitting results as plots (in ``.pdf`` format) and also
...
.. note::
   ...
   from the fitting are shown in lighter color. In plots of
