...
def update_params(params=None, map_names=None, model=None, temperature=None, p_total=None, l_total=None):
    ...
    if model == '2st.pb_kex':
        ...
        pb = ('(1.0 / (1.0 + exp(({dh_b} - {t_kelvin} * {ds_b}) / {rt})))'
              .format(kbt_h=kbt_h, t_kelvin=t_kelvin, rt=rt, **map_names))
        ...
        l_free = ('0.5 * ({l_total} - {p_total} - {kd_ab} + '
                  '(({l_total} - {p_total} - {kd_ab}) ** 2 + 4.0 * {kd_ab} * {l_total})**0.5)'
                  .format(p_total=p_total, l_total=l_total, **map_names))

        kex = ('({l_free} + {kd_ab}) * {kab}'
               .format(p_total=p_total, l_total=l_total, **map_names))

        pb = ('({l_total} - {l_free}) / {p_total}'
              .format(l_total=l_total, p_total=p_total, **map_names))
        ...
        l_free = ('0.5 * ({l_total} - {p_total} - {kd} + '
                  '(({l_total} - {p_total} - {kd}) ** 2 + 4.0 * {kd} * {l_total})**0.5)'
                  .format(p_total=p_total, l_total=l_total, **map_names))
