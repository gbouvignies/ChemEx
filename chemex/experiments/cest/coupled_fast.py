...
class Profile(base_profile.BaseProfile):
    def __init__(self, profile_name, measurements, exp_details):
        ...
        nucleus = self.resonance.atom.nucleus
        symbol = self.resonance.group.symbol.lower()
        name = self.resonance.atom.name.lower()

        self.ppm_to_rads = (
            self.h_larmor_frq * two_pi *
            constants.xi_ratio[nucleus])

        self.multiplet = util.calc_multiplet(j_couplings[symbol][name])
    ...
    def _calculate_profile(self, pb, kex, dw, r_iz, r_ixy, dr_ixy, cs):
        ...
        for b1_offset, w in zip(self.b1_offsets, ws):
            ...
            if b1_offset <= -1.0e+04:
                ...
                for j, weight in self.multiplet:
                    ...
                    sl1 = [2, 5]
                    sl2 = [i for i, ws in enumerate(s.imag) if abs(ws) < 0.1 * w1]
                    sl3 = [2]
