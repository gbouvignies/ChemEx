...
attributes_exp = ('h_larmor_frq', 'temperature', 'carrier', 'b1_frq', 'b1_inh', 'b1_inh_res', 'time_t1')
...
class Profile(base_profile.BaseProfile):
    def __init__(self, profile_name, measurements, exp_details):
        ...
        self.b1_inh = check_par(exp_details, 'b1_inh', float)
        self.b1_inh_res = check_par(exp_details, 'b1_inh_res', int)
        ...
        self.b1_frqs = np.linspace(-2.0, 2.0, self.b1_inh_res) * self.b1_inh + self.b1_frq
        self.b1_weights = stats.norm.pdf(self.b1_frqs, self.b1_frq, self.b1_inh)
    ...
    def _calculate_profile(self,
        ...
        for b1_offset in self.b1_offsets:
            ...
            if b1_offset <= -1.0e+04:
                ...
                propagator = np.average(
                    [linalg.expm(compute_liouvillian(omega1x_i=omega1, **kwargs) * self.time_t1)
                     for omega1 in self.b1_frqs * two_pi],
                    weights=self.b1_weights,
                    axis=0
                )

                magz_a = propagator.dot(magz_eq)[2, 0]
