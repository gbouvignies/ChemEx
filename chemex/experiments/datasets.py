...
class DataSet(object):
    ...
    def calculate_residuals(self, params, verbose=True, threshold=1e-3):
        ...
        residuals = np.concatenate([
                                       profile.calculate_residuals(params)
        ...
        if verbose:
            ...
            if change < -threshold:
                nvarys = len(
                    [param for param in params.values() if param.vary]
                )
    ...
    def write_chi2_to(self, params, path='./'):
        ...
        with open(filename, 'w') as f:
            ...
            f.write("chisq         = {: .5e}\n".format(chisq))
            f.write("redchi        = {: .5e}\n".format(redchi))
            f.write("chisq_p-value = {: .5e} "
                    "# Chi-squared test\n".format(chi2_p_value))
            f.write("ks_p-value    = {: .5e} "
    ...
    def add_dataset_from_file(self, filename, res_incl=None, res_excl=None):
        ...
        try:
            ...
            sys.exit("\nIn the section 'experiment' of {}, '{}' must be "
                     "provided!".format(filename, e))
        ...
        path = util.normalize_path(
            working_dir, experiment_details.get('path', './')
        )
        ...
        try:
            ...
            sys.exit("The experiment '{}', refered in '{}' is not "
                     "implemented.".format(experiment_type, filename))

        data, ndata = reading.read_profiles(
            path, profile_filenames, experiment_details, res_incl, res_excl
        )
