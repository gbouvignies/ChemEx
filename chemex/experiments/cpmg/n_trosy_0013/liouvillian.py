...
def compute_2hznz_eq(pb):
    mag_eq = zeros((12, 1))
    mag_eq[5, 0] += 1.0
    mag_eq[11, 0] += pb/(1.0 - pb)

    return mag_eq


def compute_alpha_eq(pb):
    mag_eq = zeros((12, 1))
    mag_eq[2, 0] += 0.5
    mag_eq[5, 0] += 0.5
    mag_eq[8, 0] += pb/(1.0 - pb)*0.5
    mag_eq[11, 0] += pb/(1.0 - pb)*0.5

    return mag_eq


def compute_beta_eq(pb):
    mag_eq = zeros((12, 1))
    mag_eq[2, 0] -= 0.5
    mag_eq[5, 0] += 0.5
    mag_eq[8, 0] -= pb/(1.0 - pb)*0.5
...
def get_alpha(mag):
    magz_a = mag[2, 0] + mag[5, 0]
    magz_b = mag[8, 0] + mag[11, 0]
