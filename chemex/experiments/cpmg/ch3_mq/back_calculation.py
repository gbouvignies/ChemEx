...
from liouvillian import compute_2HxCy_eq, \
    compute_liouvillian
...
@lru_cache()
def make_calc_observable(time_t2=0.0, ppm_to_rads_h=1.0, ppm_to_rads_c=1.0,
                         carrier_h=0.0, carrier_c=0.0, smallflg='Y', _id=None):
    """
    ...
    @lru_cache(1)
    def make_propagators(pb=0.0, kex=0.0, dwh=0.0, dwc=0.0, r_mq=5.0, csh_offset=0.0, csc_offset=0.0):

        l_free = compute_liouvillian(
            pb=pb, kex=kex, dwh=dwh, dwc=dwc, r_mq=r_mq,
            csh_offset=csh_offset, csc_offset=csc_offset,
        )
    ...
    @lru_cache(100)
    def _calc_observable(pb=0.0, kex=0.0, dwh=0.0, dwc=0.0, r_mq=5.0, csh=0.0, csc=0.0, ncyc=0):
        """
        ...
        csh : float
            Proton chemical shift of state A in ppm.
        csc : float
            Carbon chemical shift of state A in ppm.
        smallflg: string ['y', 'n']
        ...
        dwh *= ppm_to_rads_h
        dwc *= ppm_to_rads_c
        csh_offset = (csh - carrier_h) * ppm_to_rads_h
        csc_offset = (csc - carrier_c) * ppm_to_rads_c

        l_free, p_zeta = make_propagators(
            pb=pb, kex=kex, dwh=dwh, dwc=dwc, dwh=dwh, dwc=dwc,
            r_mq=r_mq, csh_offset=csh_offset, csc_offset=csc_offset,
        )
