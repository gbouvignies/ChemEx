...
"""13C(methyl) - H to C CPMG (3-state)
...
even ncyc should be recorded. The calculation uses a 18x18 basis set:

[Cx(a), Cy(a), Cz(a), 2HxCz(a), 2HyCz(a), 2HzCz(a),
 Cx(b), Cy(b), Cz(b), 2HxCz(b), 2HyCz(b), 2HzCz(b),
 Cx(c), Cy(c), Cz(c), 2HxCz(c), 2HyCz(c), 2HzCz(c)]
...
class Profile(base_profile.BaseProfile):
    ...
    def create_default_parameters(self):
        ...
        parameters.add_many(
            ...
            (self.map_names['j_a'], 135.0, False, None, None, None),
    ...
    def _calculate_profile(self, pb, pc, kex_ab, kex_bc, kex_ac, cs_i_a, dw_i_ab, dw_i_ac, rho_i_a, lambda_i_a,
        ...
        rhoa_i_c = lambda_i_c + rho_is_a - rho_i_a
        ...
        p_element = reduce(dot, [p_taub, p_90px, p_180x_s, p_90px, p_taub])
    ...
    def calculate_profile(self, params=None, **kwargs):
        ...
        ncycs = kwargs.get('ncyc', None)
        if ncycs is not None:
            self.ncycs, ncycs_orig = ncycs, self.ncycs
            values = self._calculate_profile.__wrapped__(**kwargs_profile)
