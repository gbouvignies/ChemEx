...
"""1H(methyl - 13CHD2) - Pure Anti-Phase Proton CPMG (3-state)
...
class Profile(base_profile.BaseProfile):
    ...
    def _calculate_profile(self, pb, pc, kex_ab, kex_bc, kex_ac, cs_i_a, dw_i_ab, dw_i_ac, rho_s_a, lambda_i_a,
        ...
        p_180pmx = 0.5 * (matrix_power(p_90px, 2) + matrix_power(p_90mx, 2))
        ...
        for ncyc, tau_cp in zip(self.ncycs, self.tau_cp_list):
            ...
            if ncyc == 0:
                ...
                mag = reduce(dot, [p_90px, p_180pmx, p_90px, mag_eq])

            else:

                p_free = p_free_list[tau_cp]
                p_cpy = matrix_power(p_free.dot(p_180py).dot(p_free), int(ncyc))
                mag = reduce(dot, [p_90px, p_neg, p_cpy, p_180pmx, p_cpy, p_neg, p_90px, mag_eq])
