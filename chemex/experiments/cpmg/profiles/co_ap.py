...
class Profile(cpmg_profile.CPMGProfile):
    ...
    def __init__(self, name, measurements, exp_details):
        ...
        self.sidechain_flg = base_profile.check_par(
            exp_details, "sidechain_flg", str
        ).lower()
        self.taucc = base_profile.check_par(
            exp_details, "taucc", float, default=9.09e-3
        )
        ...
        if "3st" in self.model:
            from chemex.spindynamics.three_state import ixyzsz

            self.base = ixyzsz
        else:
            from chemex.spindynamics.two_state import ixyzsz
        ...
        self.map_names["r1_s_a"] = parameters.ParameterName(
            "r1_a", **kwargs
        ).to_full_name()
        self.map_names["r1_s_b"] = parameters.ParameterName(
            "r1_b", **kwargs
        ).to_full_name()
        ...
        if "3st" in self.model:
            self.map_names["r1_s_c"] = parameters.ParameterName(
                "r1_c", **kwargs
            ).to_full_name()
    ...
    def calculate_unscaled_profile(self, **kwargs):
        ...
        l_pw1y = l_free + self.base.compute_liouvillian(w1y_i=+self.w1_i)
        l_mw1y = l_free + self.base.compute_liouvillian(w1y_i=-self.w1_i)
        ...
        if self.sidechain_flg == "y":
            ...
            p_flip = functools.reduce(
                np.dot, [p_90my, p_taucc, p_180pmy, p_taucc, p_90py]
            )
        ...
        for ncyc, tau_cp in zip(self.ncycs, self.tau_cp_list):
            if ncyc == 0:
                ...
                p_cpx = np.linalg.matrix_power(
                    p_free.dot(p_180x_perfect).dot(p_free), int(ncyc)
                )
