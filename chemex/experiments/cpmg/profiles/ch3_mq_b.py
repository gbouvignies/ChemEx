...
class Profile(cpmg_profile.CPMGProfile):
    ...
    def __init__(self, profile_name, measurements, exp_details):
        ...
        if '3st' in self.model:
            from chemex.bases.three_state import ixysxy
            self.base = ixysxy
        else:
            from chemex.bases.two_state import ixysxy
            self.base = ixysxy
    ...
    def calculate_unscaled_profile(self, **kwargs):
        ...
        omega_i_a, omega_i_b, omega_i_c, omega_i_d = np.array(
            [kwargs.get(key, 0.0) for key in ('cs_i_a', 'cs_i_b', 'cs_i_c', 'cs_i_d')]) * self.ppm_i

        omega_s_a, omega_s_b, omega_s_c, omega_s_d = np.array(
            [kwargs.get(key, 0.0) for key in ('cs_s_a', 'cs_s_b', 'cs_s_c', 'cs_s_d')]) * self.ppm_s
        ...
        for ncyc, tau_cp in zip(self.ncycs, self.tau_cp_list):
            if ncyc == 0:
                ...
                p_cp = np.linalg.matrix_power(np.dot(np.dot(p_free, p_180y_i), p_free), int(ncyc))
                mag = functools.reduce(np.dot, [p_cp, p_180x_s, p_cp, mag0])

            # 2HxCy[A]
            profile.append(np.float64(mag[self.base.index_iysx_b]))
